...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate http://doudou-ziyuan.com/ssr/bbr.sh; then
...
Set_config_port(){
	while true
	do
	echo -e "请输入要设置的ShadowsocksR账号 端口(1-65535)"
	read -e -p "(默认: 19527):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="19527"
...
Update_SSR(){
	SSR_installation_status
	echo -e "因豆豆暂停更新ShadowsocksR服务端，所以此功能临时禁用。"
	#cd ${ssr_folder}
	#git pull
	#Restart_SSR
}
